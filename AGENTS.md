...
Run from the repo root; `npm` or `bun` both work (`bun.lock` is present).

- `npm run dev` — Vite dev server with HMR (port 5199 for the video renderer).
- `npm run build` — `tsc -b` then `vite build` → `dist/`.
- `npm run preview` — serve the built `dist/` bundle.
- `npm run lint` — run `oxlint` (no auto-fix step).
- `node scripts/render-video.mjs <framesDir> [fps] [startSec] [endSec]` — render frames to JPEG; requires `npm run dev` on `http://localhost:5199`.
...
- **Linting**: `oxlint` with `react`, `typescript`, `oxc` plugins. `react/rules-of-hooks` = error; `react/only-export-components` = warn (`allowConstantExport: true`). Run `npm run lint` before committing.
...
No unit-test framework is configured. Validate changes by running `npm run dev`, scrubbing the timeline, and confirming each chapter renders correctly. For visual changes, export frames with `scripts/render-video.mjs` and compare.
...
- Run `npm run lint` and `npm run build` successfully before opening a PR.
